...
function optimise_weights_mask_wb_20151112164933()
...
for i = 1 : size(wdata, 1)
    ...
    tmp_w = tmp_w / min(abs(tmp_w_nonzero));
    
    op_wei_dat(i, :) = tmp_w;
    
    % write weights masks
    tmp_h1 = head{1};
    tmp_h1.fname = [sprintf('%02d', i) '_' prefix '_' num2str(thresh) '_' key_str '.nii'];
    
    cd([svml_path '/' simu_eff_folder '/' wei_folder]);
    spm_write_vol(tmp_h1, reshape(tmp_w, dims));
    
    clear tmp_w tmp_w_nonzero tmp_h1
end
