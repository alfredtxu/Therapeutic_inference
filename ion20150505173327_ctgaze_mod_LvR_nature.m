...
function ion20150505173327_ctgaze_mod_LvR_nature()
...
for h=1:length(hit)
    ...
    for l=1:size(devpair,1)
        ...
        % -W  regularization parameter lambda   (default 1)
        wW=[10^-3 10^-2 10^-1 1 10 10^2 10^3 10^4 10^5];
        
        % -U  regularization parameter lambda_u (default 1)
        uU=[10^-3 10^-2 10^-1 1 10 10^2 10^3 10^4 10^5];
                            ...
                            for ne=1:length(per_neu)
                                ...
                                for ra=1:length(tRatio)
                                    ...
                                    for iter=1:times
                                        ...
                                        training_posind=posind(tmp(1:floor(posnum*tmp_tRatio)));
                                        test_posind=posind(tmp(floor(posnum*tmp_tRatio)+1:end));
                                        ...
                                        training_negind=negind(tmp(1:floor(negnum*tmp_tRatio)));
                                        test_negind=negind(tmp(floor(negnum*tmp_tRatio)+1:end));
                                        ...
                                        training_neuind=neuind(tmp(1:floor(length(neuind)*per_neu)));
                                        ...
                                        results(iter,ne,lu,w)=accur;
                                        sensitivity(iter,ne,lu,w)=sum(test_label==valid & test_label==1)./sum(test_label==1);
                                        specificity(iter,ne,lu,w)=sum(test_label==valid & test_label==-1)./sum(test_label==-1);
