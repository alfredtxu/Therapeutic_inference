...
%
% comm.
%       * select treating cases from the * DEVIATED (labelled as -1) test data only
% 
clc
clear
close all
...
refmod = '20150914163217_ctLvN_min1_max200_pre';
...
%  'trdat_20150914163217_ctLvN_min1_max200_pre_1_19.75_3.5_2_p96_n35_A2_W1000_U1.2_S61_R0.73282_1';
hit = 1;
devia = 19.75;
norma = 3.5;
rev = 2;
...
[corrgaze_l, corrgaze_r] = ctgaze_kde();
...
for i = 1 : length(info_red)
    ...
    if tmpi.ctgazeF == 1
        ctgaze(i, 1) = tmpi.ctresc_clmp_calc(1) - corrgaze_l;
        ctgaze(i, 2) = tmpi.ctresc_clmp_calc(2) - corrgaze_r;
    else
        ctgaze(i, :) = [nan, nan];
    end
...
for i = 1 : length(ctgaze)
    if le(ctgaze(i, 1), devia*-1) && le(ctgaze(i, 2), devia*-1)
        label(i) = -1;
    elseif le(abs(ctgaze(i, 1)), norma) && le(abs(ctgaze(i, 2)), norma)
        label(i) = 1;
    elseif isnan(ctgaze(i))
        label(i) = 0;
    else
        label(i) = 9;
    end
...
for we = 1 : length(weif)
    ...
    for te = 1 : length(treatp)
        ...
        for iter = 1 : times
            ...
            tmplab = randperm(length(find(tslab == -1)));
            tmplab = tmplab(1:ceil(length(tslab) / 2));
            
            tslab_treat_p = ones(1, length(tslab)/2);
            tslab_treat_n = ones(1, length(tslab)/2) .* -1;
            
            if ~eq(tmp_treat, 0)
                tmp_treated = tmplab(1 : ceil(length(tmplab) * tmp_treat));
                tslab_treat_n(tmp_treated) = 1;    
            end
            
            tslab_treat = [tslab_treat_p, tslab_treat_n];
            ...
            pv1(iter,te, we)=stats.p(2);
            pv2(iter,te, we)=stats.p(3);
