...
%
% Tianbo XU
% init.date: 18.05.2015
function ion20150518181756_s17_G1_LvR_CT()
...
svmlin_mask=[svmlin_p '/' mask_folder];
mfile=[svmlin_mask '/ct_vxgaze_ev.nii'];
    ...
    for ga=1:size(devia,1)
        ...
        for i=1:length(info_red)
            ...
            if ctgazeF==1
                ...
                tgaze_max=max(info_red(i).ctresc_clmp_calc);
                tgaze_min=min(info_red(i).ctresc_clmp_calc);
                
                if ge(abs(tgaze_max),abs(tgaze_min))
                    tgaze=tgaze_max;
                else
                    tgaze=tgaze_min;
                end
