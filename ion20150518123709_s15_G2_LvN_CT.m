...
function ion20150518123709_s15_G2_LvN_CT()
...
for i=1:size(data,1)
    ...
    data_h(i,:,:,:)=reshape(tmphit,[31 37 31]);
...
for h=1:length(hit)
    ...
    [data_red, age_red, info_red]=ion20150430110732_reduce_dimensionality(data_h,age,info,tmp_hit);
                        ...
                        for po=1:length(portion)
                            ...
                            for ra=1:length(tRatio)
                                ...
                                for iter=1:times
                                    ...
                                    % clear the training & test data
                                    rmfiles3=dir([svmlin_datap '/' mod '/*']);
                                    for rm3=3:length(rmfiles3)
                                        system(['rm ' svmlin_datap '/' mod '/' rmfiles3(rm3).name]);
                                    end
                                end
