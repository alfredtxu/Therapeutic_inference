...
% init. 30.08.2016
% 
% comm. correlated factors: age + sex + volume
% * label: -1, 1, 0
...
refmod = '201608261209_ctmri_pre_20160830174100';
...
% 'trdat_201608261209_ctmri_pre_20160830174100_1_12_3_2_p89_n36_A2_W10000_U1.1_S53_R0.712_1'
hit = 1;
devia = 12;
norma = 3;
rev = 2;
...
[data_red, info_red, age_red, sex_red, vol_red] = ion20160830152112_reduce_dimensionality_age_sex_vol(data_v, info_v, age_v,sex_v,vol_v, hit);
...
subage = age_red(labidx, :);
subsex = sex_red(labidx, :);
...
subage_rev = subage(sort(tot_idx),:);
subsex_rev = subsex(sort(tot_idx),:);
...
for we = 1 : length(weif)
    ...
    for te = 1 : length(treatp)
        ...
        for iter = 1 : times
            ...
            tsage = subage_rev(ts_idx, :);
            tssex = subsex_rev(ts_idx, :);
            ...
            % use the prediction in a glm model that helps us isolate the treatment + age_sex_volume effect
            [b, dev, stats] = glmfit([tslab_treat(:) valid(:) tsage(:) tssex(:) tsvol(:)], treat_val(:), distr);
            ...
            [b, dev, stats] = glmfit([tslab_treat(:) tsage(:) tssex(:) tsvol(:)], treat_val(:), distr);
