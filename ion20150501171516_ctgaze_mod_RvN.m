...
function ion20150501171516_ctgaze_mod_RvN()
...
for h=1:length(hit)
    ...
    for l=1:length(devpair)
        
        tmpl=devpair(l);
        
        for n=1:size(norpair,1)
            
            tmpn1=norpair(n,1);
            tmpn2=norpair(n,2);
            
            % summarise key information of currently running mode
            str_thresh=['CT_' feature '_[inf_' num2str(tmpl) ']_[' num2str(tmpn1) '_' num2str(tmpn2) ']'];
            ...
            for i=1:length(info_red)
                ...
                if ctgazeF==1
                    ...
                    tgaze=sum(info_red(i).ctresc_clmp_calc);
                    
                    if ge(tgaze,tmpl)
                        label(i,:)=1;
                    elseif ge(tgaze,tmpn1) && le(tgaze,tmpn2)
                        label(i,:)=-1;
                    else
                        label(i,:)=9;
                    end
                                ...
                                for ne=1:length(per_neu)
                                    ...
                                    for ra=1:length(tRatio)
                                        ...
                                        for iter=1:times
                                            ...
                                            sS=floor(tmp_portion.*((abs(posnum-negnum)))*tmp_per_neu);
                                            ...
                                            results(iter)=accur;
                                            sensitivity(iter)=sum(test_label==valid & test_label==1)./sum(test_label==1);
                                            specificity(iter)=sum(test_label==valid & test_label==-1)./sum(test_label==-1);
            ...
            % end of loop: threshold - norpair
        end
        
        % end of loop: threshold - devpair
    end
    % end of loop: threshold - hit rates
end
