...
% init. 20.11.2015
% 
% comm. create the 'p-value' files separetelt for each weights file
% 
% time stamps: 20151126170053
...
refmod = '20150917165945_ctmri_pre_20151126182450_1';
...
% 'trdat_20150917165945_ctmri_pre_20151126111841_1_7_3.5_2_p113_n82_A2_W1000_U1.3_S31_R0.57949_w'
% 'trdat_20150917165945_ctmri_pre_20151126111841_1_7_3.5_2_p113_n82_A2_W10000_U1.2_S31_R0.57949_w'

% 'trdat_20150917165945_ctmri_pre_20151126105922_1_7_3.5_2_p109_n79_A2_W10000_U1.1_S30_R0.57979_1'

% 'trdat_20150917165945_ctmri_pre_20151125181521_1_10.75_3_2_p89_n42_A2_W1000_U1.2_S47_R0.67939_1'

% 'trdat_20150917165945_ctmri_pre_20151125181629_1_9.5_3.5_2_p113_n50_A2_W10000_U1.1_S63_R0.69325_3'

% 'trdat_20150917165945_ctmri_pre_20151126181748_1_8.5_3.5_2_p109_n57_A2_W10000_U1.2_S52_R0.65663_1'
% 'trdat_20150917165945_ctmri_pre_20151126182215_1_9.75_3.5_2_p113_n49_A2_W1000_U1.3_S64_R0.69753_1'
% 'trdat_20150917165945_ctmri_pre_20151126182115_1_10.5_3.25_2_p103_n43_A2_W1000_U1.1_S60_R0.70548_1'
...
for we = 1 : length(weif)
    ...
    for te = 1 : length(treatp)
        ...
        for iter = 1 : times
            ...
            pv1(iter, te)=stats.p(2);
            pv2(iter, te)=stats.p(3);
            ...
            pv_vm1(iter, te)=stats.p(2);
            pv_vm2(iter, te)=stats.p(3);
            pv_vm3(iter, te)=stats.p(4);
            
            disp(['pv_vm1: ' num2str(stats.p(2))])
            disp(['pv_vm2: ' num2str(stats.p(3))])
            disp(['pv_vm3: ' num2str(stats.p(4))])
            ...
            pvm1(iter, te)=stats.p(2);
            pvm2(iter, te)=stats.p(3);
            
            disp(['pvm1: ' num2str(stats.p(2))])
            disp(['pvm2: ' num2str(stats.p(3))])
    ...
    save([pval_path '/' modname '/pv1_' num2str(we) '.mat'], 'pv1');
    save([pval_path '/' modname '/pv2_' num2str(we) '.mat'], 'pv2');
    
    save([pval_path '/' modname '/pv_vm1_' num2str(we) '.mat'], 'pv_vm1');
    save([pval_path '/' modname '/pv_vm2_' num2str(we) '.mat'], 'pv_vm2');
    save([pval_path '/' modname '/pv_vm3_' num2str(we) '.mat'], 'pv_vm3');
    
    save([pval_path '/' modname '/pvm1_' num2str(we) '.mat'], 'pvm1');
    save([pval_path '/' modname '/pvm2_' num2str(we) '.mat'], 'pvm2');
    
    save([pval_path '/' modname '/p_' num2str(we) '.mat'], 'p');
    
    clear pv1 pv2 pv_vm1 pv_vm2 pv_vm3 pvm1 pvm2 p
