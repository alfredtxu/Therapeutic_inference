function ion20150519170517_s008_G4_LvN_CTMRI()
...
% standard binary zeta dataset
load('zeta1333_6mm_bin.mat');

% information strut
load('stroke_info_light.mat');

% other patients related detail vector
load('age_all.mat');
toc
...
svmlin_mask=[svmlin_p '/' mask_folder];
mfile=[svmlin_mask '/ct_vxgaze_mu.nii'];
hitm=spm_read_vols(spm_vol(mfile));
hitm=hitm(:);
hitm_min=min(hitm);
hitm_max=max(hitm);
...
for i=1:length(hitm)
    ...
    if sign(hitm(i))==-1
        hitm_nor(i,:)=hitm(i)./hitm_min;
    elseif sign(hitm(i))==1
        hitm_nor(i,:)=hitm(i)./hitm_max;
    else
        hitm_nor(i,:)=0;
    end
end
...
for i=1:size(data,1)
    ...
    tmphit=tmpdata(:).*hitm_nor;
    
    datah(i,:,:,:)=reshape(tmphit,[31 37 31]);
    clear tmpdata tmphit
end
...
for h=1:length(hit)
    ...
    [data_red, datah_red, age_red, info_red]=ion20150515165848_reduce_dimensionality3(data,datah,age,info,tmp_hit);
        ...
        for no=1:length(normals)
            ...
            for i=1:length(info_red)
                ...
                if ctgazeF==1 && t2gazeF==1
                    ...
                    if le(ctgaze,tmp_dev) && gt(abs(t2gaze),tmp_nor)
                        label(i,:)=1;
                    elseif le(ctgaze,tmp_dev) && le(abs(t2gaze),tmp_nor)
                        label(i,:)=-1;
                    else
                        label(i,:)=9;
                    end
                        ...
                        for po=1:length(portion)
                            ...
                            for ra=1:length(tRatio)
                                ...
                                for iter=1:times
                                    ...
                                    sS=floor(tmp_portion.*abs(posnum-negnum));
                                    ...
                                    if gt(posnum,negnum)
                                        ...
                                        training_index=[training_posind;training_negind;posind_cov];
                                    ...
                                    elseif posnum==negnum
                                        ...
                                        % combined training indeices
                                        training_index=[training_posind;training_negind];
                                    ...
                                    elseif lt(posnum,negnum)
                                        ...
                                        % combined training indeices
                                        training_index=[training_posind;training_negind;negind_cov];
                                    end
