...
%
% Tianbo XU
% init.date: 14.05.2015
function ion20150514144043_s103_G4_LvN_CT_MRI()

%% comments
%
% gaze: CT resc_clmp_calc + MRI resc_calc
% data: image data + age + volume + duration
% deviation: -25:-17
% normal: 
% hit rate: 7
% training ratio:0.8
...
for h=1:length(hit)
    ...
    [data_red, age_red, duration_red, info_red]=ion20150513104717_reduce_dimensionality2(data,age,duration,info,tmp_hit);
    ...
    for ga=1:length(devia)
        
        tmp_dev=devia(ga);
        
        for no=1:length(normals)
            ...
            for i=1:length(info_red)
                ...
                t2gazeF=info_red(i).t2gazeF;
                
                if ctgazeF==1 && t2gazeF==1 
                    
                    ctgaze_max=max(info_red(i).ctresc_clmp_calc);
                    ctgaze_min=min(info_red(i).ctresc_clmp_calc);
                    ctgaze_abs=abs(info_red(i).ctresc_clmp_calc);
                    
                    t2gaze_max=max(info_red(i).t2resc_calc);
                    t2gaze_min=min(info_red(i).t2resc_calc);
                    t2gaze_abs=abs(info_red(i).t2resc_calc);
                    
                    if le(ctgaze_min,tmp_dev) && lt(t2gaze_min,tmp_nor*-1)
                        label(i,:)=1;
                    elseif le(max(t2gaze_abs),tmp_nor)
                        label(i,:)=-1;
                    else
                        label(i,:)=9;
                    end
                ...
                clear ctgazeF t2gazeF ctgaze t2gaze
            end
                        ...
                        for po=1:length(portion)
                            ...
                            for ra=1:length(tRatio)
                                ...
                                for iter=1:times
                                    ...
                                    training_dura=subdura(training_index,:);
                                    ...
                                    training_data_corr=[training_data training_age training_vol training_dura];
                                    ...
                                    results(iter,ra,po,lu,w,ar,no,ga,h)=accur;
                                    sensitivity(iter,ra,po,lu,w,ar,no,ga,h)=sum(test_label==valid & test_label==1)./sum(test_label==1);
                                    specificity(iter,ra,po,lu,w,ar,no,ga,h)=sum(test_label==valid & test_label==-1)./sum(test_label==-1);
