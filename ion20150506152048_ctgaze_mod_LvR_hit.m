...
function ion20150506152048_ctgaze_mod_LvR_hit()
...
%% variables
% key mode feature
mod='20150507103644_CT_LvR3_[inf_-10]_[10_inf]_185_201_264';
...
for h=1:length(hit)
    ...
    for l=1:size(devpair,1)
        ...
        % -U  regularization parameter lambda_u (default 1)
        uU=[1 1.1 1.2 1.3 1.5 10 100 1000];
                            ...
                            for ne=1:length(per_neu)
                                ...
                                for ra=1:length(tRatio)
                                    ...
                                    for iter=1:times
                                        ...
                                        results(iter,lu,h)=accur;
                                        sensitivity(iter,lu,h)=sum(test_label==valid & test_label==1)./sum(test_label==1);
                                        specificity(iter,lu,h)=sum(test_label==valid & test_label==-1)./sum(test_label==-1);
