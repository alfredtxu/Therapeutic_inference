% summary: generating weights mask
%
% Tianbo XU
% init.date: 21.05.2015
function ion20150521171000_s014_G1_LvR_CT()
...
% key mode feature
mod=['s015_G1_LvR_CT_' stamp];

% svmlin realted paths
svmlin_p='/media/tianbo/2TB_1/z_matlab_workspace/pro_Therapeutic/f1_svmlin_2015';

data_folder='ana_data';
wei_folder='ana_weights';
res_folder='ana_results';

svmlin_datap=[svmlin_p '/' data_folder];
svmlin_weip=[svmlin_p '/' wei_folder];
svmlin_resp=[svmlin_p '/' res_folder];

system(['mkdir ' svmlin_datap '/' mod]);
system(['mkdir ' svmlin_weip '/' mod]);
                        ...
                        for ra=1:length(tRatio)
                            ...
                            for iter=1:times
                                ...
                                for rm1=1:length(rmfiles1)
                                    system(['mv ' rmfiles1(rm1).name ' ' svmlin_weip '/' mod]);
                                end
                                for rm2=1:length(rmfiles2)
                                    system(['mv ' rmfiles2(rm2).name ' ' svmlin_weip '/' mod]);
                                end
                            end
