...
%          simulating treatment on partial cases, plus lesion volumes 
%          to see how senstive the treated cases affecting the accuracy. 
%
% Author. Tianbo XU
...
% init. 20.11.2015
% 
% comm. create each weights-file separetely
%       exp(beta): odds-ratio in Logistic regression
% 20.12.2015 distr: binomial
...
modname = [time_stamp '_post_' refmod];
...
for i = 1 : length(info_red)
    ...
    if tmpi.ctgazeF == 1 && tmpi.t2gazeF == 1
        ctgaze(i, 1) = tmpi.ctresc_clmp_calc(1) - corrgaze_ct_r;
        ctgaze(i, 2) = tmpi.ctresc_clmp_calc(2) - corrgaze_ct_l;
        
        mrigaze(i, 1) = tmpi.t2resc_calc(1) - corrgaze_mri_r;
        mrigaze(i, 2) = tmpi.t2resc_calc(2) - corrgaze_mri_l;
    else
        ctgaze(i, :) = [nan, nan];
        mrigaze(i, :) = [nan, nan];
    end
...
for we = 1 : length(weif)
    ...
    for te = 1 : length(treatp)
        ...
        for trial = 1 : times
            ...
            beta_v_logit{trial, te} = b;
            dev_v_logit{trial, te} = dev;
            stats_v_logit{trial, te} = stats;
            clear b dev stats
            
            [b, dev, stats] = glmfit([tslab_treat(:) valid(:)], treat_val(:), distr, 'link', 'probit');
            beta_v_probit{trial, te} = b;
            dev_v_probit{trial, te} = dev;
            stats_v_probit{trial, te} = stats;
            clear b dev stats
            
            % use the prediction in a glm model that helps us isolate the treatment + volume effect
            [b, dev, stats] = glmfit([tslab_treat(:) valid(:) tsvol(:)], treat_val(:), distr);
            beta_vm_logit{trial, te} = b;
            dev_vm_logit{trial, te} = dev;
            stats_vm_logit{trial, te} = stats;
            clear b dev stats
            
            [b, dev, stats] = glmfit([tslab_treat(:) valid(:) tsvol(:)], treat_val(:), distr, 'link', 'probit');
            beta_vm_probit{trial, te} = b;
            dev_vm_probit{trial, te} = dev;
            stats_vm_probit{trial, te} = stats;
            clear b dev stats
            
            % use the prediction in a glm model that helps us isolate the volume effect
            [b, dev, stats] = glmfit([tslab_treat(:) tsvol(:)], treat_val(:), distr);
            beta_m_logit{trial, te} = b;
            dev_m_logit{trial, te} = dev;
            stats_m_logit{trial, te} = stats;
            clear b dev stats
            
            [b, dev, stats] = glmfit([tslab_treat(:) tsvol(:)], treat_val(:), distr, 'link', 'probit');
            beta_m_probit{trial, te} = b;
            dev_m_probit{trial, te} = dev;
            stats_m_probit{trial, te} = stats;
            clear b dev stats
            
            % compare without the prediction
            [b, dev, stats] = glmfit(tslab_treat(:), treat_val(:), distr);
            beta_plain_logit{trial, te} = b;
            dev_plain_logit{trial, te} = dev;
            stats_plain_logit{trial, te} = stats;
            clear b dev stats
            
            [b, dev, stats] = glmfit(tslab_treat(:), treat_val(:), distr, 'link', 'probit');
            beta_plain_probit{trial, te} = b;
            dev_plain_probit{trial, te} = dev;
            stats_plain_probit{trial, te} = stats;
            clear b dev stats
    ...
    % save 'logit'
    save([pval_path '/' modname '/beta_v_logit_' num2str(we) '.mat'], 'beta_v_logit');
    save([pval_path '/' modname '/beta_vm_logit_' num2str(we) '.mat'], 'beta_vm_logit');
    save([pval_path '/' modname '/beta_m_logit_' num2str(we) '.mat'], 'beta_m_logit');
    save([pval_path '/' modname '/beta_logit_' num2str(we) '.mat'], 'beta_plain_logit');
    
    save([pval_path '/' modname '/dev_v_logit_' num2str(we) '.mat'], 'dev_v_logit');
    save([pval_path '/' modname '/dev_vm_logit_' num2str(we) '.mat'], 'dev_vm_logit');
    save([pval_path '/' modname '/dev_m_logit_' num2str(we) '.mat'], 'dev_m_logit');
    save([pval_path '/' modname '/dev_logit_' num2str(we) '.mat'], 'dev_plain_logit');
    
    save([pval_path '/' modname '/stats_v_logit_' num2str(we) '.mat'], 'stats_v_logit');
    save([pval_path '/' modname '/stats_vm_logit_' num2str(we) '.mat'], 'stats_vm_logit');
    save([pval_path '/' modname '/stats_m_logit_' num2str(we) '.mat'], 'stats_m_logit');
    save([pval_path '/' modname '/stats_logit_' num2str(we) '.mat'], 'stats_plain_logit');
    
    clear beta_v_logit dev_v_logit stats_v_logit
    clear beta_vm_logit dev_vm_logit stats_vm_logit
    clear beta_m_logit dev_m_logit stats_m_logit
    clear beta_plain_logit dev_palin_logit stats_plain_logit
    
    % save 'probit'
    save([pval_path '/' modname '/beta_v_probit_' num2str(we) '.mat'], 'beta_v_probit');
    save([pval_path '/' modname '/beta_vm_probit_' num2str(we) '.mat'], 'beta_vm_probit');
    save([pval_path '/' modname '/beta_m_probit_' num2str(we) '.mat'], 'beta_m_probit');
    save([pval_path '/' modname '/beta_probit_' num2str(we) '.mat'], 'beta_plain_probit');
    
    save([pval_path '/' modname '/dev_v_probit_' num2str(we) '.mat'], 'dev_v_probit');
    save([pval_path '/' modname '/dev_vm_probit_' num2str(we) '.mat'], 'dev_vm_probit');
    save([pval_path '/' modname '/dev_m_probit_' num2str(we) '.mat'], 'dev_m_probit');
    save([pval_path '/' modname '/dev_probit_' num2str(we) '.mat'], 'dev_plain_probit');
    
    save([pval_path '/' modname '/stats_v_probit_' num2str(we) '.mat'], 'stats_v_probit');
    save([pval_path '/' modname '/stats_vm_probit_' num2str(we) '.mat'], 'stats_vm_probit');
    save([pval_path '/' modname '/stats_m_probit_' num2str(we) '.mat'], 'stats_m_probit');
    save([pval_path '/' modname '/stats_probit_' num2str(we) '.mat'], 'stats_plain_probit');
    
    clear beta_v_probit dev_v_probit stats_v_probit
    clear beta_vm_probit dev_vm_probit stats_vm_probit
    clear beta_m_probit dev_m_probit stats_m_probit
    clear beta_plain_probit dev_palin_probit stats_plain_probit
